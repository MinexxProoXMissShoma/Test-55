...
def fresh_default_data():
    return {
        ...
        "participants": {},  # uid -> {"username","name","joined_ts","verified_ok_at_join":bool}
        "verify_targets": [],  # [{"ref","display"}]
        ...
        # backrules (leave lock)
        "backrules_mode": "off",   # "on" / "off"
        "backrules_locked": {},    # uid -> {"username","locked_bd_time","reason"}
        "backrules_unbanned": {},  # uid -> {"bd_time"}

        # manager logs
        "events": [],  # list of {"bd_time","type","user_id","username","detail"}
    }
...
def bd_now_str():
    return bd_now().strftime("%d/%m/%Y %H:%M:%S")
...
def log_event(event_type: str, user_id: str = "", username: str = "", detail: str = ""):
    with lock:
        ev = {
            "bd_time": bd_now_str(),
            "type": event_type,
...
def verify_user_join(bot, user_id: int) -> (bool, str):
    """
    returns (ok, failed_target_display)
    """
...
# =========================================================
# POPUPS (copy admin only where you wanted)
# =========================================================
...
def popup_access_locked_copy_admin() -> str:
    # 200 chars-ish, clean + copy admin
    return (
        "🚫 ACCESS LOCKED\n\n"
        "You left the required channels/groups.\n"
        "Entry is restricted for this giveaway.\n\n"
        "Copy Admin Username:\n"
        f"{ADMIN_CONTACT}"
    )
...
def live_tick(context: CallbackContext):
    with lock:
        ...
        if remaining <= 0:
            ...
            log_event("giveaway_closed_auto", "", "", f"title={data.get('title','')} participants={participants_count()}")
...
def start_draw_admin(context: CallbackContext, admin_chat_id: int):
    ...
    ctx = {"chat_id": admin_chat_id, "msg_id": msg.message_id, "start_ts": utc_now_ts(), "tick": 0}
    ...
    def tick(job_ctx: CallbackContext):
        ...
        try:
            job_ctx.bot.edit_message_text(jd["chat_id"], jd["msg_id"], build_draw_progress_text(percent, dots, spin))
        except Exception:
            pass
...
def draw_finalize_admin(context: CallbackContext):
    ...
    admin_chat_id = jd["chat_id"]
    admin_msg_id = jd["msg_id"]
    ...
    log_event("draw_completed_admin_preview", str(ADMIN_ID), ADMIN_CONTACT, f"winners={len(data.get('winners',{}))}")

    try:
        context.bot.edit_message_text(admin_chat_id, admin_msg_id, pending_text, reply_markup=winners_approve_markup())
    except Exception:
        context.bot.send_message(admin_chat_id, pending_text, reply_markup=winners_approve_markup())


# =========================================================
# AUTO DRAW IN CHANNEL + PIN
# =========================================================
def stop_auto_draw():
...
def start_auto_draw_in_channel(context: CallbackContext):
    ...
    m = context.bot.send_message(chat_id=CHANNEL_ID, text=build_draw_progress_text(0, ".", SPINNER[0]))
    ...
    ctx = {"msg_id": m.message_id, "start_ts": utc_now_ts(), "tick": 0}
    ...
    log_event("auto_draw_started_channel", "", "", "progress_post_pinned")
...
def auto_draw_finalize(context: CallbackContext):
    stop_auto_draw()
    mid = context.job.context["msg_id"]

    # delete progress message (pin auto gone)
    try:
    ...
    log_event("auto_draw_completed_channel", "", "", f"winners={len(data.get('winners',{}))}")
...
def reset_finalize(context: CallbackContext):
    ...
    jd = context.job.context
    log_event("full_reset_done", str(ADMIN_ID), ADMIN_CONTACT, "all_removed")
...
def cmd_start(update: Update, context: CallbackContext):
    ...
    if u and u.id == ADMIN_ID:
        ...
        log_event("admin_start", str(ADMIN_ID), ADMIN_CONTACT, "cmd=/start")
    else:
...
def cmd_autowinnerpost(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "Choose:\n"
        "✅ ON → close হলে auto draw + channel winners post\n"
...
def cmd_backrules(update: Update, context: CallbackContext):
    if not is_admin(update):
        return
    mode = data.get("backrules_mode", "off").upper()
    update.message.reply_text(
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "🔐 BACK RULES\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Current: {mode}\n\n"
        "ON → required verify target থেকে বের হলে user LOCK হবে\n"
        "OFF → lock system বন্ধ\n"
        "UNBAN → user_id দিয়ে unlock",
...
def cmd_manager(update: Update, context: CallbackContext):
    ...
    # default today
    target_date = bd_now().date()
    if arg:
        try:
            dt = datetime.strptime(arg, "%d/%m/%Y")
            target_date = dt.date()
        except Exception:
            update.message.reply_text("Use format: /manager DD/MM/YYYY\nExample: /manager 07/01/2026")
            return

    events = data.get("events", []) or []
    lines = []
    lines.append(f"MANAGER REPORT (BD) - {target_date.strftime('%d/%m/%Y')}")
    lines.append("=" * 50)
    for ev in events:
        try:
            bd_time = ev.get("bd_time", "")
            dt = datetime.strptime(bd_time, "%d/%m/%Y %H:%M:%S")
        ...
        lines.append(f"[{ev.get('bd_time','')}] {ev.get('type','')}")
        uid = ev.get("user_id", "")
        un = ev.get("username", "")
        if uid or un:
            lines.append(f"USER: {un} | {uid}")
        if ev.get("detail", ""):
            lines.append(f"DETAIL: {ev.get('detail','')}")
        lines.append("-" * 50)

    if len(lines) <= 2:
        lines.append("No logs found for this date.")

    # write txt
    filename = f"manager_{target_date.strftime('%d_%m_%Y')}.txt"
...
def cmd_newgiveaway(update: Update, context: CallbackContext):
    ...
    with lock:
        ...
        keep_backmode = data.get("backrules_mode", "off")
        keep_locked = data.get("backrules_locked", {})
        keep_unb = data.get("backrules_unbanned", {})
        keep_events = data.get("events", [])
        ...
        data["backrules_mode"] = keep_backmode
        data["backrules_locked"] = keep_locked
        data["backrules_unbanned"] = keep_unb
    ...
    log_event("newgiveaway_started", str(ADMIN_ID), ADMIN_CONTACT, "")
...
def cmd_blockpermanent(update: Update, context: CallbackContext):
    ...
    log_event("permanent_block_setup_opened", str(ADMIN_ID), ADMIN_CONTACT, "")
...
def cmd_blockoldwinner(update: Update, context: CallbackContext):
    ...
    log_event("oldwinner_block_setup_opened", str(ADMIN_ID), ADMIN_CONTACT, "")
...
def cmd_completePrize(update: Update, context: CallbackContext):
    ...
    log_event("delivery_proof_started", str(ADMIN_ID), ADMIN_CONTACT, "")
...
def admin_text_handler(update: Update, context: CallbackContext):
    ...
    if admin_state == "add_verify":
        ...
        with lock:
            ...
            save_data()
        log_event("verify_target_added", str(ADMIN_ID), ADMIN_CONTACT, f"target={ref}")
        update.message.reply_text(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if admin_state == "remove_verify_pick":
        ...
        with lock:
            ...
            if n == 11:
                ...
                save_data()
                admin_state = None
                log_event("verify_targets_removed_all", str(ADMIN_ID), ADMIN_CONTACT, "")
            ...
            data["verify_targets"] = targets
            save_data()
        log_event("verify_target_removed", str(ADMIN_ID), ADMIN_CONTACT, f"removed={removed.get('display','')}")
        admin_state = None
        update.message.reply_text(
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if admin_state == "old_winner_mode":
        ...
        update.message.reply_text("Send old winners list (one per line): @username | user_id")
        return
    ...
    if admin_state == "perma_block_list":
        ...
        with lock:
            ...
            save_data()
        log_event("permanent_block_added", str(ADMIN_ID), ADMIN_CONTACT, f"count={len(entries)}")
        admin_state = None
        update.message.reply_text("✅ Permanent block list updated successfully!")
        return

    # OLDWINNER BLOCK manual
    if admin_state == "oldwinner_block_list_manual":
        ...
        with lock:
            ...
            data["old_winners"] = ow
            save_data()
        log_event("oldwinner_block_added", str(ADMIN_ID), ADMIN_CONTACT, f"count={len(entries)}")
        admin_state = None
        update.message.reply_text("✅ Old winner block list updated successfully!")
        return
    ...
    if admin_state == "backrules_unban_input":
        raw = msg.replace(" ", "")
        if not raw.isdigit():
            update.message.reply_text("Send User ID only.")
            return
        with lock:
            locked = data.get("backrules_locked", {}) or {}
            if raw in locked:
                del locked[raw]
                data["backrules_locked"] = locked
                data["backrules_unbanned"][raw] = {"bd_time": bd_now_str()}
                save_data()
        log_event("backrules_unban", str(ADMIN_ID), ADMIN_CONTACT, f"user_id={raw}")
        admin_state = None
        update.message.reply_text("✅ Unbanned successfully. User can join again.")
        return

    # DELIVERY text (DONE handled elsewhere; photos handled separately)
    if admin_state == "delivery_collect_photos":
        if msg.upper() == "DONE":
            ...
            admin_state = None
            update.message.reply_text("✅ Ready. Approve to post in channel?", reply_markup=delivery_approve_markup())
            return
...
def cb_handler(update: Update, context: CallbackContext):
    ...
    # Any button click -> permanent/oldwinner block popup
    if uid != str(ADMIN_ID):
        if uid in (data.get("permanent_block", {}) or {}):
            log_event("blocked_click_permanent", uid, uname, f"button={qd}")
            try:
        ...
        if data.get("old_winner_mode") == "block" and uid in (data.get("old_winners", {}) or {}):
            log_event("blocked_click_oldwinner", uid, uname, f"button={qd}")
            try:
    ...
    # backrules toggle
    if qd in ("backrules_on", "backrules_off", "backrules_unban"):
        if uid != str(ADMIN_ID):
            return
        if qd == "backrules_on":
            with lock:
                ...
                save_data()
            log_event("backrules_on", str(ADMIN_ID), ADMIN_CONTACT, "")
            try:
        ...
        if qd == "backrules_off":
            with lock:
                ...
                save_data()
            log_event("backrules_off", str(ADMIN_ID), ADMIN_CONTACT, "")
            try:
        ...
        if qd == "backrules_unban":
            ...
            try:
                query.edit_message_text("Send User ID to UNBAN (unlock access):")
            except Exception:
                pass
            return
    ...
    if qd in ("autopost_on", "autopost_off"):
        ...
        with lock:
            ...
            save_data()
        log_event("autopost_toggle", str(ADMIN_ID), ADMIN_CONTACT, f"value={data['auto_winner_post']}")
        try:
            query.edit_message_text("✅ Auto Winner Post: ON" if data["auto_winner_post"] else "❌ Auto Winner Post: OFF")
        except Exception:
            pass
        return
    ...
    if qd.startswith("preview_"):
        ...
        if qd == "preview_approve":
            ...
            log_event("giveaway_posted_channel", str(ADMIN_ID), ADMIN_CONTACT, f"title={data.get('title','')}")
            try:
        ...
        if qd == "preview_reject":
            log_event("giveaway_preview_rejected", str(ADMIN_ID), ADMIN_CONTACT, "")
            try:
    ...
    if qd == "end_confirm":
        ...
        stop_live()
        log_event("giveaway_closed_manual", str(ADMIN_ID), ADMIN_CONTACT, "")
    ...
    if qd == "reset_confirm":
        if uid != str(ADMIN_ID):
            return
        log_event("reset_confirmed", str(ADMIN_ID), ADMIN_CONTACT, "")
        try:
    ...
    if qd == "reset_reject":
        if uid != str(ADMIN_ID):
            return
        log_event("reset_rejected", str(ADMIN_ID), ADMIN_CONTACT, "")
        try:
    ...
    if qd == "delivery_approve":
        ...
        try:
            ...
            log_event("delivery_posted_channel", str(ADMIN_ID), ADMIN_CONTACT, f"photos={len(photos)}")
    ...
    if qd == "delivery_reject":
        ...
        with lock:
            ...
            data["delivery_caption"] = ""
            save_data()
        log_event("delivery_rejected", str(ADMIN_ID), ADMIN_CONTACT, "")
        try:
    ...
    if qd == "join_giveaway":
        ...
        # backrules lock check (only when ON)
        if data.get("backrules_mode") == "on":
            locked = data.get("backrules_locked", {}) or {}
            if uid in locked:
                log_event("access_locked_click", uid, uname, "already_locked")
                try:
        ...
        ok, failed_target = verify_user_join(context.bot, int(uid))
        if not ok:
            # backrules ON => lock user permanently for this giveaway until admin unban
            if data.get("backrules_mode") == "on":
                with lock:
                    locked = data.get("backrules_locked", {}) or {}
                    locked[uid] = {
                        "username": uname,
                        "locked_bd_time": bd_now_str(),
                        "reason": f"verify_failed:{failed_target}",
                    }
                    data["backrules_locked"] = locked
                    save_data()
                log_event("access_locked_set", uid, uname, f"failed_target={failed_target}")
                try:
                    query.answer(popup_access_locked_copy_admin(), show_alert=True)
                except Exception:
                    pass
            ...
            # normal verify required
            log_event("verify_failed", uid, uname, f"failed_target={failed_target}")
            try:
        ...
        # first winner click again => same popup
        first_uid = str(data.get("first_winner_id") or "")
        if first_uid and uid == first_uid:
            u = uname or (data.get("first_winner_username", "") or "@username")
            log_event("first_winner_click_again", uid, u, "")
            try:
                query.answer(popup_first_winner(u, uid), show_alert=True)
            except Exception:
                pass
            return
        ...
        if uid in (data.get("participants", {}) or {}):
            log_event("already_joined_click", uid, uname, "")
            try:
        ...
        with lock:
            ...
            data["participants"][uid] = {
                "username": uname,
                "name": full_name,
                "joined_ts": utc_now_ts(),
                "verified_ok_at_join": True,
            }
            save_data()

        log_event("joined_giveaway", uid, uname, f"first={is_first}")

        # instant update live post once (keep job 5s)
        try:
            live_mid = data.get("live_message_id")
            start_ts = data.get("start_time")
            if live_mid and start_ts:
                ...
                context.bot.edit_message_text(chat_id=CHANNEL_ID, message_id=live_mid, text=build_live_text(remaining), reply_markup=join_button_markup())
        except Exception:
            pass
    ...
    if qd == "winners_approve":
        ...
        save_current_winners_to_history()
        log_event("winners_posted_channel", str(ADMIN_ID), ADMIN_CONTACT, f"count={len(data.get('winners',{}))}")
    ...
    if qd == "winners_reject":
        ...
        with lock:
            data["pending_winners_text"] = ""
            save_data()
        log_event("winners_rejected", str(ADMIN_ID), ADMIN_CONTACT, "")
        try:
    ...
    # CLAIM PRIZE
    if qd == "claim_prize":
        # if backrules ON and locked => locked popup
        if data.get("backrules_mode") == "on":
            locked = data.get("backrules_locked", {}) or {}
            if uid in locked and uid != str(ADMIN_ID):
                log_event("claim_blocked_access_locked", uid, uname, "")
                try:
                    query.answer(popup_access_locked_copy_admin(), show_alert=True)
                except Exception:
                    pass
        ...
        if uid in winners:
            ...
            if deadline and utc_now_ts() > float(deadline):
                log_event("claim_expired", uid, uname, "")
                try:
            ...
            wuname = winners.get(uid, {}).get("username", "") or "@username"
            log_event("claim_winner_clicked", uid, wuname, "")
            try:
                query.answer(popup_claim_winner(wuname, uid), show_alert=True)
            except Exception:
                pass
        else:
            log_event("claim_not_winner", uid, uname, "")
            try:
...
def main():
    ...
    print("Bot is running (PTB v13, Final v2) ...")
