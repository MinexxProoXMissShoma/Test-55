...
def fresh_default_data():
    return {
        ...
        "delivered_winners": {},  # uid(str) -> {"username": "", "ts": float}
    }
...
def build_progress(percent: float) -> str:
    ...
    blocks = 9  # তোমার স্টাইল অনুযায়ী 9 blocks
...
def popup_join_success(username: str, uid: str) -> str:
    return (
        ...
        f"👤 {username}\n"
        f"🆔 {uid}\n\n"
        "Please stay until winners announced."
    )
...
def popup_prize_expired() -> str:
    return (
        "⏳ PRIZE EXPIRED\n"
        "Your 24-hour claim time ended.\n"
        "This prize is no longer available."
    )


# =========================================================
# GIVEAWAY TEXT BUILDERS (✅ NEW STYLE)
# Title only (no extra emoji/text added)
# Border lines fixed
# =========================================================
...
def build_winners_post_text() -> str:
    with lock:
        ...
        first_uid = str(data.get("first_winner_id") or "") or ""
        title = (data.get("title") or "").strip()
        prize = (data.get("prize") or "").strip()
        total = len(winners) if winners else int(data.get("winner_count", 0) or 0)
        delivered_count = len(delivered)
        first_uname = (winners.get(first_uid, {}) or {}).get("username", "") or data.get("first_winner_username", "") or "@username"
...
# =========================================================
# DELIVERY SYSTEM (✅ /prizedelivery)
# Bot asks input -> works only with UserID; username optional
# =========================================================
def refresh_winners_post(context: CallbackContext):
    ...
    try:
        context.bot.edit_message_text(
            ...
            message_id=mid,
            text=text,
            reply_markup=claim_button_markup() if data.get("claim_expires_ts") and now_ts() < float(data["claim_expires_ts"]) else None
        )
    except Exception:
        pass
...
def cmd_reset(update: Update, context: CallbackContext):
    if not is_admin(update):
        return
    update.message.reply_text("Reset system? (Not implemented demo-safe)")
...
def admin_text_handler(update: Update, context: CallbackContext):
    ...
    if admin_state == "title":
        with lock:
            data["title"] = msg  # ✅ Title only (no emoji added)
            save_data()
        admin_state = "prize"
    ...
    if admin_state == "duration":
        ...
        if seconds <= 0:
            update.message.reply_text("Invalid duration. Example: 3 Minute")
            return
        with lock:
    ...
    if admin_state == "prizedelivery_input":
        ...
        with lock:
            ...
            # optional: store username if winners has empty username
        ...
        admin_state = None
        update.message.reply_text(f"✅ Prize delivered marked!\n🆔 {uid}")
        return
...
def cb_handler(update: Update, context: CallbackContext):
    ...
    if qd == "join_giveaway":
        if not data.get("active"):
            query.answer("This giveaway is not active.", show_alert=True)
            return
    ...
    if qd == "winners_approve":
        ...
        try:
            ...
            query.edit_message_text("✅ Approved & posted winners to channel.")
        except Exception as e:
            try:
    ...
    if qd == "winners_reject":
        ...
        with lock:
            ...
            save_data()
        try:
            query.edit_message_text("❌ Rejected winners list.")
        except Exception:
            pass
        return
...
def main():
    ...
    print("Bot is running (PTB 13, GSM hosting friendly) ...")
